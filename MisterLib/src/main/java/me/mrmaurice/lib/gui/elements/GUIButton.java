...
import me.mrmaurice.lib.utils.Reflections.Version;
...
public class GUIButton {
	...
	public GUIButton onClick(Consumer<InventoryClickEvent> cons) {
		action = cons;
		return this;
	}
	...
	public ItemStack toItemStack() {
		...
		ItemStack item;
		if (Reflections.getVersion() != Version.v13) {
			mat = Material.valueOf(type.toUpperCase());
			item = new ItemStack(mat, amount, (short) data);
		} else {
			MaterialData md = new MaterialData(Material.valueOf("LEGACY_" + type), data);
			mat = Reflections.getNetClass("CraftLegacy").getMethod("fromLegacy", MaterialData.class).invoke(null, md);
		...
		if (im instanceof SkullMeta)
			if (profile != null) {
				...
				Reflections.getField(sm.getClass(), "profile").set(sm, profile);
				item.setItemMeta(sm);
			}
		return item;
